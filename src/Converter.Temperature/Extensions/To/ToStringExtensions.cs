...
public static class ToStringExtensions
{
        ...
        where TInput : TemperatureBase
    {
        ...
        {
            nameof(Celsius) when input is CelsiusString castInput => Parser(
                castInput.Temperature,
                CelsiusDoubleConversions.CelsiusToCelsius,
                fractionalCount),
            nameof(Celsius) when input is FahrenheitString castInput => Parser(
                castInput.Temperature,
                FahrenheitDoubleConversions.FahrenheitToCelsius,
                fractionalCount),
            nameof(Celsius) when input is KelvinString castInput => Parser(
                castInput.Temperature,
                KelvinDoubleConversions.KelvinToCelsius,
                fractionalCount),
            nameof(Celsius) when input is GasString castInput => Parser(
                castInput.Temperature,
                GasDoubleConversion.GasToCelsius,
                fractionalCount),
            nameof(Celsius) when input is RankineString castInput => Parser(
                castInput.Temperature,
                RankineDoubleConversions.RankineToCelsius,
                fractionalCount),
            nameof(Fahrenheit) when input is CelsiusString castInput => Parser(
                castInput.Temperature,
                CelsiusDoubleConversions.CelsiusToFahrenheit,
                fractionalCount),
            nameof(Fahrenheit) when input is FahrenheitString castInput => Parser(
                castInput.Temperature,
                FahrenheitDoubleConversions.FahrenheitToFahrenheit,
                fractionalCount),
            nameof(Fahrenheit) when input is KelvinString castInput => Parser(
                castInput.Temperature,
                KelvinDoubleConversions.KelvinToFahrenheit,
                fractionalCount),
            nameof(Fahrenheit) when input is GasString castInput => Parser(
                castInput.Temperature,
                GasDoubleConversion.GasToFahrenheit,
                fractionalCount),
            nameof(Fahrenheit) when input is RankineString castInput => Parser(
                castInput.Temperature,
                RankineDoubleConversions.RankineToFahrenheit,
                fractionalCount),
            nameof(Kelvin) when input is CelsiusString castInput => Parser(
                castInput.Temperature,
                CelsiusDoubleConversions.CelsiusToKelvin,
                fractionalCount),
            nameof(Kelvin) when input is FahrenheitString castInput => Parser(
                castInput.Temperature,
                FahrenheitDoubleConversions.FahrenheitToKelvin,
                fractionalCount),
            nameof(Kelvin) when input is KelvinString castInput => Parser(
                castInput.Temperature,
                KelvinDoubleConversions.KelvinToKelvin,
                fractionalCount),
            nameof(Kelvin) when input is GasString castInput => Parser(
                castInput.Temperature,
                GasDoubleConversion.GasToKelvin,
                fractionalCount),
            nameof(Kelvin) when input is RankineString castInput => Parser(
                castInput.Temperature,
                RankineDoubleConversions.RankineToKelvin,
                fractionalCount),
            nameof(Gas) when input is CelsiusString castInput => Parser(
                castInput.Temperature,
                CelsiusDoubleConversions.CelsiusToGas,
                fractionalCount),
            nameof(Gas) when input is FahrenheitString castInput => Parser(
                castInput.Temperature,
                FahrenheitDoubleConversions.FahrenheitToGas,
                fractionalCount),
            nameof(Gas) when input is KelvinString castInput => Parser(
                castInput.Temperature,
                KelvinDoubleConversions.KelvinToGas,
                fractionalCount),
            nameof(Gas) when input is GasString castInput => Parser(
                castInput.Temperature,
                GasDoubleConversion.GasToGas,
                fractionalCount),
            nameof(Gas) when input is RankineString castInput => Parser(
                castInput.Temperature,
                RankineDoubleConversions.RankineToGas,
                fractionalCount),
            nameof(Rankine) when input is CelsiusString castInput => Parser(
                castInput.Temperature,
                CelsiusDoubleConversions.CelsiusToRankine,
                fractionalCount),
            nameof(Rankine) when input is FahrenheitString castInput => Parser(
                castInput.Temperature,
                FahrenheitDoubleConversions.FahrenheitToRankine,
                fractionalCount),
            nameof(Rankine) when input is KelvinString castInput => Parser(
                castInput.Temperature,
                KelvinDoubleConversions.KelvinToRankine,
                fractionalCount),
            nameof(Rankine) when input is GasString castInput => Parser(
                castInput.Temperature,
                GasDoubleConversion.GasToRankine,
                fractionalCount),
            nameof(Rankine) when input is RankineString castInput => Parser(
                castInput.Temperature,
